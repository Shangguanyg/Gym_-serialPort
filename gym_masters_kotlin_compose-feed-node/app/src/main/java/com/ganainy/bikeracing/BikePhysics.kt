package com.ganainy.bikeracing
...
class BikePhysics(private val world: World) {
    ...
    private lateinit var frontWheelJoint: PrismaticJoint
    private lateinit var rearWheelJoint: PrismaticJoint
    ...
    private fun attachWheels() {
        // 使用PrismaticJoint替代RevoluteJoint
        val frontJointDef = PrismaticJointDef().apply {
            initialize(bikeBody, frontWheelBody, frontWheelBody.worldCenter, Vector2(1f, 0f))
            enableMotor = true
            maxMotorForce = MOTOR_TORQUE * currentBikeStats.acceleration
            motorSpeed = -MAX_MOTOR_SPEED * currentBikeStats.topSpeed
        }
        frontWheelJoint = world.createJoint(frontJointDef) as PrismaticJoint

        val rearJointDef = PrismaticJointDef().apply {
            initialize(bikeBody, rearWheelBody, rearWheelBody.worldCenter, Vector2(1f, 0f))
            enableMotor = true
            maxMotorForce = MOTOR_TORQUE * currentBikeStats.acceleration
            motorSpeed = -MAX_MOTOR_SPEED * currentBikeStats.topSpeed
        }
        rearWheelJoint = world.createJoint(rearJointDef) as PrismaticJoint
    }

    fun accelerate(speedMultiplier: Float = 1f) {
        val speed = MAX_MOTOR_SPEED * currentBikeStats.topSpeed * speedMultiplier
        (frontWheelJoint as PrismaticJoint).motorSpeed = -speed
        (rearWheelJoint as PrismaticJoint).motorSpeed = -speed
    }
    fun brake() {
        (frontWheelJoint as PrismaticJoint).motorSpeed = MAX_MOTOR_SPEED * 0.5f
        (rearWheelJoint as PrismaticJoint).motorSpeed = MAX_MOTOR_SPEED * 0.5f
    }
