...
class Boards(private val skiaLayer: SkiaLayer, view: View): SkikoRenderDelegate {
    ...
    val footImage = Image.makeFromEncoded( this::class.java.getResourceAsStream("/foot.png")?.readBytes()
    ?: throw IllegalArgumentException("无法加载 foot.png"))

    val srcRect = Rect.makeWH(footImage.width.toFloat(), footImage.height.toFloat())

    val scaledWidth = footImage.width * 3/4f

    val scaledHeight = footImage.height * 2/3f
    ...
    private val buttonLayout: LinearLayout = LinearLayout(view.context)

    // 用于存储点的坐标
    private val points = mutableListOf<Pair<Float, Float>>() // 存储点的坐标
    private var isDrawing = false // 标志以避免重复绘制
    ...
    init {
        ...
        layoutParams.setMargins(20, 20, 20, 20) // 设置边距

        // 创建和配置按钮 1
        val button1 = Button(view.context).apply {
            text = "按钮 1"
            setOnClickListener {
                println("按钮 1 被点击")
            }
        ...
        // 创建和配置按钮 2
        val button2 = Button(view.context).apply {
            text = "按钮 2"
            setOnClickListener {
                println("按钮 2 被点击")
            }
        ...
        // 将按钮添加到布局
        buttonLayout.addView(button1, layoutParams)
        buttonLayout.addView(button2, layoutParams)

        // 将按钮布局添加到传入的视图中
        (view as ViewGroup).addView(buttonLayout)
    ...
    // 启动定时器以每 0.5 秒添加一个点
    private fun startDrawingPoints() {
        timerRunnable = object : Runnable {
            override fun run() {
                if (isDrawing) {
                    // 在直径为690的圆形范围内随机生成点
                    val radius = 180f // 圆的半径
                    val randomRadius = (0..radius.toInt()).random().toFloat() // 随机半径
                    val angle = (0..360).random() // 随机角度

                    // 计算 x 和 y 坐标，确保在圆内
                    val x = 365 + randomRadius * Math.cos(Math.toRadians(angle.toDouble())).toFloat()
                    val y = 365 + randomRadius * Math.sin(Math.toRadians(angle.toDouble())).toFloat()
                    points.add(Pair(x, y)) // 添加到点列表
                }
                timerHandler.postDelayed(this, 600) // 每 0.5 秒调用
            }
        }
        isDrawing = true // 开始绘制点
        timerHandler.post(timerRunnable!!) // 启动定时器
    }

    // 停止绘制点的逻辑（可选）
    fun stopDrawingPoints() {
        isDrawing = false // 停止生成点
        timerRunnable?.let { timerHandler.removeCallbacks(it) } // 停止计时器
    }
            ...
            for (y in 30 + platformYOffset.toInt() .. height - 200 step 200) {
                ...
                val hover =
                    (xpos - xOffset) / scale > x &&
                            (xpos - xOffset) / scale < x + 200 &&
                            (ypos - yOffset) / scale > y &&
                            (ypos - yOffset) / scale < y + 200
...
//
//
//                canvas.drawRect(Rect.makeXYWH(x + 10f, y + 5f, 120f, 160f), fill)
                //绘制轮廓圆形: 在相同位置绘制一个宽高均为 40 的圆形轮廓，使用 stroke 作为轮廓样式。
                //canvas.drawOval(Rect.makeXYWH(x + 5f, y + 5f, 40f, 40f), stroke)
//                canvas.drawRect(Rect.makeXYWH(x + 10f, y + 5f, 120f, 160f), stroke)
//
//                canvas.drawRect(Rect.makeXYWH(x + 10f, y + 170f, 120f, 100f), fill)
//                canvas.drawRect(Rect.makeXYWH(x + 10f, y + 170f, 120f, 100f), stroke)
...
//                        (y.toFloat() / width * 5000).toLong()
        ...
        val fill = watchFill
        val stroke = watchStroke

        // 绘制按钮的 Paint 对象
        val buttonNormalPaint = Paint().apply {
            color = 0xFF4285F4.toInt()  // 蓝色
            mode = PaintMode.FILL
        }
        val buttonHoverPaint = Paint().apply {
            color = 0xFF3367D6.toInt()  // 深蓝色
            mode = PaintMode.FILL
        }
        val buttonPressedPaint = Paint().apply {
            color = 0xFF2E5CB8.toInt()  // 更深的蓝色
            mode = PaintMode.FILL
        }
        val buttonStrokePaint = Paint().apply {
        ...
        canvas.drawOval(Rect.makeXYWH(20f, 20f, 690f, 690f), orangeFill)
        canvas.drawOval(Rect.makeXYWH(20f, 20f, 690f, 690f), stroke)

        canvas.drawOval(Rect.makeXYWH(135f, 135f, 460f, 460f), yellowFill)
        canvas.drawOval(Rect.makeXYWH(135f, 135f, 460f, 460f), stroke)

        canvas.drawOval(Rect.makeXYWH(250f, 250f, 230f, 230f), greenFill)
        canvas.drawOval(Rect.makeXYWH(250f, 250f, 230f, 230f), stroke)


        canvas.drawRect(Rect.makeXYWH(820f, 70f, 240f, 320f), fill)
        canvas.drawRect(Rect.makeXYWH(820f, 70f, 240f, 320f), stroke)
        canvas.drawRect(Rect.makeXYWH(820f, 400f, 240f, 200f), fill)
        canvas.drawRect(Rect.makeXYWH(820f, 400f, 240f, 200f), stroke)

        canvas.drawRect(Rect.makeXYWH(1100f, 70f, 240f, 320f), fill)
        canvas.drawRect(Rect.makeXYWH(1100f, 70f, 240f, 320f), stroke)
        canvas.drawRect(Rect.makeXYWH(1100f, 400f, 240f, 200f), fill)
        canvas.drawRect(Rect.makeXYWH(1100f, 400f, 240f, 200f), stroke)

        canvas.drawImageRect(footImage, srcRect, dstRect)

        //canvas.drawImage(footImage, 720f, 10f)

        canvas.drawLine(20f, 365f, 710f, 365f, stroke)

        canvas.drawLine(365f, 20f, 365f, 710f, stroke)

        // 绘制所有点
        for (point in points) {
            val (x, y) = point
            canvas.drawCircle(x, y, 2f, Paint().apply { color = 0xFFFF0000.toInt() }) // 绘制红色点
        }
        ...
        if (points.size > 1) {
            for (i in 1 until points.size) {
                ...
                val (x2, y2) = points[i]
                canvas.drawLine(x1, y1, x2, y2, Paint().apply { color = 0xFFFF0000.toInt(); strokeWidth = 1f }) // 绘制白色线
            }
        }
