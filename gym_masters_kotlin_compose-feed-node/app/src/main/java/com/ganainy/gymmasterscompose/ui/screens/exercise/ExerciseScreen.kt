...
//这段代码展示了一个关于锻炼的用户界面，定义了 ExerciseScreen 组件和其配套的内容组件 ExerciseListContent。我们将分别分析这些函数及其功能。
//ExerciseScreen.kt 文件是一个 Jetpack Compose 屏幕，用于显示有关特定锻炼的详细信息。它的工作原理如下：
//内部结构
//该文件包含两个主要的可组合函数，它们构成了屏幕的架构：
...
@Composable
fun ExerciseScreen(exercise: StaticExercise, navigateBack: () -> Unit) {
