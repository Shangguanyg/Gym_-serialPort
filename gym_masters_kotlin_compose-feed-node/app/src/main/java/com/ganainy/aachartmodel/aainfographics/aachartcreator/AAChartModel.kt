...
enum class AAChartType(val value: String) {
    Column("column"), //Column series display one column per value along an X axis. 柱状图系列沿X轴为每个数值显示一个柱子。
    Bar("bar"), //A bar series is a special type of column series where the columns are horizontal. 条形图是一种特殊的柱状图，其柱子是水平排列的。
    Area("area"), //The area series type. 面积图系列类型。
    Areaspline("areaspline"), //The area spline series is an area series where the graph between the points is smoothed into a spline. //面积样条系列是一种面积系列，其中点与点之间的图形被平滑成样条曲线。
    Line("line"), //A line series displays information as a series of data points connected by straight line segments. //折线图以一系列数据点的形式显示信息，这些数据点之间由直线段连接。
    Spline("spline"), //A spline series is a special type of line series, where the segments between the data points are smoothed. //样条曲线系列是一种特殊的线条系列，其中数据点之间的线段经过平滑处理。
    Scatter("scatter"), //A scatter plot uses cartesian coordinates to display values for two variables for a set of data. //散点图使用笛卡尔坐标系来显示一组数据中两个变量的值。
    Pie("pie"), //A pie chart is a circular graphic which is divided into slices to illustrate numerical proportion. //饼状图是一种圆形图表，它被分成若干个扇形区域，用于表示数值比例。
