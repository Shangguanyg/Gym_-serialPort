...
import com.ganainy.gymmasterscompose.databinding.ActivityMultiSerialBinding;
import com.ganainy.serialportlibrary.MultiSerialPortManager;
...
import com.ganainy.serialportlibrary.stick.AbsStickPackageHelper;
import com.ganainy.serialportlibrary.stick.BaseStickPackageHelper;
import com.ganainy.serialportlibrary.stick.SpecifiedStickPackageHelper;
import com.ganainy.serialportlibrary.stick.StaticLenStickPackageHelper;
...
public class MultiSerialPortActivity extends AppCompatActivity {
    ...
    // ActivityMultiSerialBinding是ViewBinding生成的类，用于绑定布局文件中的视图。

    private ActivityMultiSerialBinding binding;
    // statusText是显示状态信息的TextView
    private TextView statusText;
    
    // 串口设备列表
    // mDevices是存储串口设备路径的数组，
    // mBaudrates、mDatabits、mParitys、mStopbits分别是设备的可用波特率、数据位、校验位和停止位的数组。
    ...
    //globalDatabits、globalParity、globalStopbits用于存储全局串口参数的默认值。
    private int globalDatabits = 8;
    private int globalParity = 0;
    private int globalStopbits = 1;
    
    // 各串口的配置
    // 定义了多个SerialPortConfig实例，每个实例对应不同的串口设备（GPS、传感器、Modbus、自定义设备）。
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //用DataBindingUtil将布局文件activity_multi_serial绑定到当前Activity。
    ...
    //视图组件：通过binding对象找到并绑定状态文本（tvStatus）。
    //按钮点击事件：为每个按钮设置点击事件，包括刷新串口、打开所有串口、关闭所有串口、发送测试数据及清除日志。
    //串口开关：为每个设备的开关按钮设置点击事件，控制对应串口的开关状态。
    ...
    private void sendTestData() {
        //通过SimpleSerialPortManager.multi()获取一个可以管理多个串口的管理器实例
        ...
        //状态更新：通过updateStatus()方法更新界面显示的信息，指明发送数据的串口数量。
    ...
    private void updateStatus(String message) {
        //检查statusText是否为null，若不为null，则将新的状态信息添加到当前文本的最前面。
        if (statusText != null) {
            String currentText = statusText.getText().toString();
            String newText = message + "\n" + currentText;
            // 保持最多20行
            // 对状态文本进行处理，只保留最新的20行信息，避免文本过长。
     ...
     *
     */
    // 字节数组转十六进制字符串：定义一个方法，将字节数组转换为十六进制字符串格式，方便在日志中查看二进制数据内容。
     ...
     * 演示动态更新粘包处理器
     */
    //updateStickyPacketExample是一个公共方法，接受一个View类型的参数view，用于从布局中的视图组件调用（如按钮）

    public void updateStickyPacketExample(View view) {
        //调用SimpleSerialPortManager.multi()获取一个多串口管理器的实例，允许对多个串口进行管理。
        MultiSerialPortManager manager = SimpleSerialPortManager.multi();
    ...
    public void serialRoutingExample() {
        //获取管理器实例：再次使用管理器来管理串口
        MultiSerialPortManager manager = SimpleSerialPortManager.multi();
        
        // 主控制串口：接收外部命令并路由到其他串口
//        打开主控制串口：
//        打开MAIN_CTRL这个串口，指定设备路径和波特率。
//        使用Builder设置粘包处理器为SpecifiedStickPackageHelper，即使用\r\n来分包。
        manager.openSerialPort("MAIN_CTRL", "/dev/ttyS0", 115200,
            new MultiSerialPortManager.SerialPortConfig.Builder()
                ...
                .build(),
            null,
            //定义OnSerialPortDataCallback，重写onDataReceived()方法
            new MultiSerialPortManager.OnSerialPortDataCallback() {
                @Override
                //当接收到数据时，将数据转换为字符串并去除前后空白
                public void onDataReceived(String serialId, byte[] data) {
                    ...
                    // 根据命令前缀路由到不同串口
                    // 命令路由：根据命令前缀（GPS:、SENSOR:、MODBUS:）决定路由逻辑：
     ...
     * 初始化设备列表
     */
    //initDevices用于初始化串口设备列表
    private void initDevices() {
        //创建SerialPortFinder实例：使用此类查找设备路径
        SerialPortFinder serialPortFinder = new SerialPortFinder();
        //getAllDevicesPath()获取连接的所有串口设备，如果没有设备，则设置为一个包含默认消息的字符串数组
        ...
        // 初始化默认配置
        // 对于gpsConfig、sensorConfig、modbusConfig和customConfig，
        // 根据找到的设备数量设置设备路径和波特率。
     ...
     * 设置全局参数下拉框
     */
    //用于设置全局参数的下拉框（数据位、校验位和停止位）
    private void setupGlobalParamSpinners() {
        // 数据位配置
        // 设置数据位选择器
        // 调用setupSpinner()方法，传入数据位的数组和一个监听器，当选择项变化时更新globalDatabits并关闭所有已打开的串口
        ...
        // 校验位配置
        // 与数据位配置相似，设置校验位，更新globalParity并关闭所有已打开的串口。
     ...
     * 关闭所有已打开的串口（参数变更时使用）
     */
    //用于关闭所有打开的串口，防止参数变化后导致问题
    private void closeAllOpenedPorts() {
        //管理器实例：获取多串口管理器的实例
        MultiSerialPortManager manager = SimpleSerialPortManager.multi();
        //使用hasOpenPorts布尔变量记录是否有打开的串口
        boolean hasOpenPorts = false;

        //关闭已打开串口：
        //依次检查四个串口的状态，如果打开则调用管理器的closeSerialPort()方法关闭，更新状态为未打开
        if (gpsOpened) {
        ...
        //如果有已关闭的串口，调用updateButtonStates()和updateStatus()方法，告知用户已经关闭所有串口
        if (hasOpenPorts) {
            updateButtonStates();
     ...
     * 设置下拉框
     */
    //setupSpinners用于设置每个串口设备和波特率的下拉框
    private void setupSpinners() {
        // GPS串口配置
        // 调用setupSpinner()为GPS设备和波特率设置下拉框，并传递监听器以处理选中事件，如果当前打开，则提醒用户重新打开串口。
    ...
    private void setupSpinner(Spinner spinner, String[] data, int defaultSelection, OnSpinnerItemSelectedListener listener) {
        ...
        //设置选择监听器：处理选中事件，调用传入的监听器的onItemSelected()方法。
     ...
     */
//    方法定义：刷新串口设备的列表。
//    重新初始化设备：调用initDevices()以更新设备列表。
//    重新设置下拉框：调用setupSpinners()以反映设备列表的变化。
//    状态更新：更新状态文本，指明找到的设备数量。
...
//    方法定义：关闭所有打开的串口。
//    获取管理器：获取多串口管理器的实例。
//    关闭所有串口：调用closeAllSerialPorts()方法关闭所有串口。
//    更新串口状态：将所有串口的状态设置为关闭。
//    更新按钮状态：调用updateButtonStates()更新按钮显示。
//    状态更新：显示“所有串口已关闭”信息。
