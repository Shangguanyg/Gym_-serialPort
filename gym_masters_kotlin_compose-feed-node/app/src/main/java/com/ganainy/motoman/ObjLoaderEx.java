...
//            StillSubMesh subMesh = new StillSubMesh(group.name, mesh, GL20.GL_TRIANGLES);
...
//            model.subMeshes[g] = subMesh;
    ...
    // 移除 @Override 注解，更改返回类型为 Model
    public Model load (FileHandle handle) {
        return loadObj(handle, false); // 默认不翻转 V 坐标
    }
}
...
class MtlLoader {
	...
	public void load (String name, FileHandle textureDir) {
		...
		Color shininesscolor = Color.WHITE;
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				if (tokens[0].length() == 0) {
                    ...
                    mat.set(ColorAttribute.createAmbient(ambcolor));
                    mat.set(ColorAttribute.createDiffuse(difcolor));
                    mat.set(ColorAttribute.createSpecular(speccolor));
                    materials.add(mat);
					...
					shininesscolor = Color.WHITE;
					texture = emptyTexture;
				} else if (
						tokens[0].toLowerCase().equals("ka") ||
						tokens[0].toLowerCase().equals("kd") ||
						tokens[0].toLowerCase().equals("ks")) // diffuse or specular
				{
					...
					if (tokens[0].toLowerCase().equals("kd")) {
						...
						ambcolor = new Color();
						ambcolor.set(r, g, b, a);
					}
				} else if (tokens[0].toLowerCase().equals("ns")) {
					float r = 1.0f / Float.parseFloat(tokens[1]);
					float g = r;
					float b = g;
					float a = 1;
					shininesscolor = new Color();
					shininesscolor.set(r, g, b, a);
        ...
        Material mat = new Material(curMatName,
                new TextureAttribute(TextureAttribute.Diffuse, texture),
                ColorAttribute.createAmbient(ambcolor),
                ColorAttribute.createDiffuse(difcolor),
                ColorAttribute.createSpecular(speccolor));
	...
	public Material getMaterial (String name) {
        ...
        for (Material mat : materials) {
            if (mat.id.equals(name)) return mat;
        }
