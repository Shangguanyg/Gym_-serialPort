...
public class MeshOptimized {
	...
	private class MeshContext implements IMeshContext {
		...
		public void render(ShaderProgram shader, int copies) {
			...
			for (int i = 0; i < nRenderContext; ++i) {
				...
				Gdx.gl20.glDrawElements(primitiveType, rc.count * copies, GL20.GL_UNSIGNED_SHORT, rc.offset * 2);
			}
		}
	}
	...
	private CreateOptimizedMeshResult createOptimizedMesh(Set<Mesh> meshes, int copies) {
		...
		for (Mesh mesh : meshes) {
            ...
            short[] indice = new short[numIndices];
            if (numIndices > 0) {
                try {
                    ...
                    // 如果获取索引失败，创建空的索引数组
                    indice = new short[0];
                    Gdx.app.log("MeshOptimized", "Failed to get indices for mesh, using empty array", e);
                }
            }
	...
	public IMeshContext add(Model model, int copies) {
        ...
        for (Node node : model.nodes) {
            for (NodePart nodePart : node.parts) {
                Set<Mesh> meshSet = matMap.get(nodePart.material);
                if (meshSet == null) {
                    meshSet = new HashSet<Mesh>();
                    matMap.put(nodePart.material, meshSet);
                }
                meshSet.add(nodePart.meshPart.mesh);
            }
        }
