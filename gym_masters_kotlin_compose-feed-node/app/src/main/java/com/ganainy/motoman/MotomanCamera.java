...
import com.ganainy.motoman.track.TrackDirection;
...
public class MotomanCamera extends PerspectiveCamera implements ITrackee {
	...
	public void followMotorcycle(float delta) {
		if (!motorcycle.state.isCrashed) {
			...
			tmpVec5.set(state.shake).scl(state.shakeness * shakeFactor);
			...
			if (state.vanishingPointNeedReset) {
				...
				if (stepFullLen > 0) {
					...
					if (step < stepFullLen) {
						tmpVec.scl(1f/stepFullLen).scl(step);
			...
			tmpVec.y = 0;
			tmpVec2.set(0, 1, 0);
			tmpVec6.set(state.vanishingPointSmoothed).sub(tmpVec).scl(-1).add(tmpVec);
