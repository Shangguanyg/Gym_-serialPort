    ...
    public static void initResource() {
        ...
        for (Node node : model.nodes) {
            for (NodePart nodePart : node.parts) {
                int nAttributes = nodePart.material.size();
                for (int i = 0; i < nAttributes; ++i) {
                    Attribute att = nodePart.material.get(i);
                    if (att instanceof TextureAttribute) {
                        TextureAttribute ta = (TextureAttribute)att;
                        ta.textureDescription.texture.setWrap(TextureWrap.Repeat, TextureWrap.Repeat);
                    }
                }
            }
        }
