...
public class FishingGameView extends SurfaceView implements Callback,Runnable{
    ...
    public FishingGameView(Context context) {
        super(context);
        // 实例SurfaceView
        for (int i = 0; i < 11; i++) {
...
//        }
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        for (int i = 0; i < 10; i++) {
            fishArray.add(new Fish(getWidth(), getHeight(), bmp[2][0])); // 使用不同类型的鱼
        }
