...
class SpaceDude : ApplicationAdapter() {
    ...
    private var gdxHeight = 0
    private var gdxWidth = 0
    private var topTubeWidth = 0
    private var bottomTubeWidth = 0
    private var topTubeHeight = 0
    private var bottomTubeHeight = 0
    ...
    override fun create() {
        ...
        characterCircle = Circle()
        font = BitmapFont()
        font.color = Color.WHITE
        font.data.setScale(10f)

        spaceCharacters = arrayOf(Texture("spacedude.png"), Texture("spacedude2.png"))
        ...
        random = Random()
        distanceBetweenTubes = gdxWidth * 0.75f
    ...
    override fun render() {
        ...
        if (gameState == 1) {
                ...
                if (tubeX[scoringTube] < gdxWidth / 2f) {
                score++
            ...
            for (i in 0 until numberOfTubes) {
                ...
                if (tubeX[i] < -topTubeWidth) {
                    ...
                    tubeOffset[i] = (random.nextFloat() - 0.5f) * (gdxHeight - GAP - 200f)
                } else {
                    tubeX[i] = tubeX[i] - TUBE_VELOCITY
                }

                batch.draw(topTube, tubeX[i], gdxHeight / 2f + GAP / 2f + tubeOffset[i])
                batch.draw(bottomTube,
                        tubeX[i],
                        gdxHeight / 2f - GAP / 2f - bottomTubeHeight + tubeOffset[i])

                topTubeRectangles[i] = Rectangle(tubeX[i],
                        gdxHeight / 2f + GAP / 2f + tubeOffset[i],
        ...
        batch.draw(spaceCharacters[flapState], gdxWidth / 2f - spaceCharacters[flapState].width / 2f, characterY)
        font.draw(batch, score.toString(), 100f, 200f)
        characterCircle.set(gdxWidth / 2f,
                characterY + spaceCharacters[flapState].height / 2f,
                spaceCharacters[flapState].width / 2f)

        for (i in 0 until numberOfTubes) {
            if (Intersector.overlaps(characterCircle, topTubeRectangles[i])
                    || Intersector.overlaps(characterCircle, bottomTubeRectangles[i])) gameState = 2
        }
    ...
    private fun startGame() {
        characterY = gdxHeight / 2f - spaceCharacters[0].height / 2f

        for (i in 0 until numberOfTubes) {
            tubeOffset[i] = (random.nextFloat() - 0.5f) * (gdxHeight - GAP - 200f)
            tubeX[i] = gdxWidth / 2f - topTubeWidth / 2f + gdxWidth + i * distanceBetweenTubes
